...
CameraStreamError LiveOutputStream::WriteInterleavedPacket(const AVPacket* Packet)
{
	...
	if (_NormalizeNoBFrameTimestamps && PacketCopy.duration == 0 && _LastPacketDuration > 0)
	...
	if (_OutputSegmentStartDTS == AV_NOPTS_VALUE)
	...
	{
		_CurrentPartialKeyframeSeekSafe = _CurrentPartialKeyframeSeekSafe && PacketKeyframeSeekSafe;
	}
